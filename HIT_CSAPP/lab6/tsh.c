...
{
	char * cmd = argv[0];
	/* check the first character to save time */
	switch(argv[0][0])
	{
		case 'q':
			/* dont need to return */
...
void waitfg(pid_t pid)
{
	...
	while(fgpid(jobs) == pid)
		/* sleep(0) is ok but it may waste time */
		sigsuspend(&mask);
    return;
}
...
void sigchld_handler(int sig) 
{
	...
	while((pid = waitpid(-1, &status, WUNTRACED | WNOHANG)) > 0)
	{
        ...
        if(WIFSTOPPED(status))
        {
          	/*put message*/
        	sio_puts("Job [");
        	sio_puti(pid2jid(pid));
        	sio_puts("] (");
        	sio_puti(pid);
        	sio_puts(") stopped by signal ");
        	sio_puti(WSTOPSIG(status));
        	sio_puts("\n");
