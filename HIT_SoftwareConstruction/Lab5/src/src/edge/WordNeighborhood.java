...
public class WordNeighborhood extends Edge {
	...
	/**
	 * AF : vertices is ArrayList, use vertices.get(0) to represent source
	 * vertex; use vertices.get(1) to represent target vertex. the label is in
	 * format : source->target.
	 * 
	 * RI : edge is valid only when vertices.size == 2; weight > 0
	 * 
	 * safety from rep exposure : label private and final, the weight is
	 * settable but will always fit RI. all sourceVertices and targetVertices
	 * methods will only return the clones set.
	 */
	...
	public boolean valid() {
		return vertices.size() == 2 && getWeight() > 0;
	}
	 ...
	 *         null
	 */
	public static WordNeighborhood wrap(String source, String target,
			double weight) {
		return wrap(source + "->" + target,
				Arrays.asList(Word.wrap(source), Word.wrap(target)), weight);
	}
	 ...
	 *         null
	 */
	public static WordNeighborhood wrap(String label, List<Vertex> vertices,
			double weight) {
