...
public class SameMovieHyperEdge extends Edge {
	 ...
	 * 
	 * @param vertices
	 *            list of vertices to add, all vertices must be instance of
	 *            Actor.
	 * 
	 ...
	 *            edge's label
	 * @param vertices
	 *            vertices to add, the vertices.size() >= 2 and the elements
	 *            should be Instance of Actor.
	 * @param weight
	 *            weight of edge, will not be used
	 * @return return generated Instance of SameMovieHyperEdge if the parameters
	 *         are valid, else return null instead
	 */
	public static SameMovieHyperEdge wrap(String label, List<Vertex> vertices,
			double weight) {
		// invalid size
		if (vertices.size() < 2)
			return null;
