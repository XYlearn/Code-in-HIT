...
public class SocialNetwork extends ConcreteGraph {
	...
	public static List<Class<? extends Vertex>> vertexWhiteList = Arrays
			.asList(Person.class);
	public static List<Class<? extends Edge>> edgeWhiteList = Arrays
			.asList(Tie.class);
	...
	protected void checkRep() {
		...
		// for (Edge edge : edges)
		// sum += edge.getWeight();
		// assert Math.abs(sum - 1.0) <= 0.00000000001;
		// }
	}

	/**
	 * Add an edge to graph. if the edge has been in graph or it has vertex
	 * which doesn't exists in graph, it will not be added and return false. the
	 * edge's weight will be set to 1 if it's the only edge in graph, otherwise
	 * after the edge is added, all edge weight will be updated to wb * (1 - wa)
	 * where wb is the original weight of edges in the graph, wa is the added
	 * edge's weight.
	 * 
	 * @param edge
	 *            Edge to add
	...
	public boolean addEdgeDirect(Edge edge) {
		// edge already exists in graph
		if (edges.contains(edge))
			return false;
		// invalid weight
	 ...
	 * @param edge
	 *            Edge to remove
	 * @return if the edge exists in graph and is removed successfully, return
	 *         true
	 */
	@Override
	...
	/**
	 * Get weight of vertex. Weight of vertex represent the influence of a
	 * Person in the SocialNetwork
	 * 
	 * @param vertex
	 *            Vertex in Graph
	 * @return return weight of graph if it exists in Graph, else return a
	 *         negative number
	 */
	...
	@Override
	public boolean allowDuplicateEdge() {
		return true;
	}
}
