...
public class ConcreteGraph implements Graph<Vertex, Edge> {
	...
	/**
	 * AF : use a HashSet of Vertex to represent vertices of graph; use a
	 * Hashset of Edge to represent edges of graph
	 * 
	 * RI : vertices is a set with no same vertices; edges is a set with no same
	 * vertices. Every vertex in edge is in vertices
	 * 
	 * safety from rep exposure: the Collection fields are protected and final,
	 * and can't be accessed from irrelevant classes, because of defensive copy.
	 */
	...
	protected void checkRep() {
		// for(Edge edge : edges)
		// assert vertices.containsAll(edge.vertices());
	}

	/**
	 * check if the graph is multi-graph
	 * 
	 * @return return true if the graph is multi-graph
	 */
	...
	public boolean addVertex(Vertex vertex) {
		boolean res = vertices.add(vertex.clone());
		checkRep();
		return res;
	}
	...
	public boolean addEdge(Edge edge) {
		...
		boolean res = edges.add(edge.clone());
		checkRep();
		return res;
	}
