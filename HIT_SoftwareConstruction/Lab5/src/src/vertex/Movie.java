...
public class Movie extends Vertex {
	 ...
	 * 
	 * safety from rep exposure: all added fields are private. and not
	 * modifiable
	 */
	...
	/**
	 * Add detail information of Movie.The information is release date, country
	 * it comes from and the grade on IMDb
	 * 
	 * @param args
	 *            the String format of arguments to fill. args[0] is release
	 *            Date, format in Decimal number string in range of [1900,
	 *            2018]; args[1] is country the Movie is froml; args[2] is
	 *            Movie's grade on IMDb
	 * 
	 * @exception IllegalArgumentException
	 *                raised when the arguments are not legal, and will do
	 *                nothing if this occurs
	 */
	@Override
	public void fillVertexInfo(String[] args) throws IllegalArgumentException {
		if (args.length < 3)
			throw new IllegalArgumentException(
					"Invalid Argument. Expected at least 3 arguments");
		// parse releaseDate
		...
		try {
			...
			throw new IllegalArgumentException(
					"Invalid Argument. Date is expected an integer in range[1900,2018]");
		}
		// get contry
		this.country = args[1];
		...
		try {
			...
			throw new IllegalArgumentException(
					"Invalid Argument. Grade is expected an float number in range[0,10]");
		}
		checkRep();
	}
	...
	@Override
	public Movie clone() {
		// String[] args = new String[] { String.valueOf(releaseDate), country,
	...
	public List<String> getVertexInfo() {
		...
		res.add(String.valueOf(getReleaseDate()));
		res.add(getCountry());
		res.add(String.valueOf(getGrade()));
		return res;
	}
}
