...
public class NetworkConnectionTest extends EdgeTest {
	 ...
	 * Testing Strategies
	 * 
	 * for addVertices: test add valid vertices and invalid vertices, test add the
	 * first time and the second time
	 * 
	 * for sourceVertices and targetVertices: covers called when the edge is valid
	 * 
	 * for clone: test whether the returned instance has same field to the original
	 * one, and test for invalid edge
	 * 
	...
	@Override
	protected NetworkConnection edgeInstance(String label, List<Vertex> vertices, double weight) {
		return NetworkConnection.wrap(label, vertices, weight);
	}

	/**
	 * Test method for {@link edge.NetworkConnection#addVertices(java.util.List)}.
	 * test add valid vertices and invalid vertices, test add the first time and the
	 * second time
	 */
	...
	/**
	 * Test method for {@link edge.NetworkConnection#sourceVertices()}.covers called
	...
	/**
	 * Test method for {@link edge.NetworkConnection#targetVertices()}.covers called
	 * when the edge is valid
	 */
	@Test
	 ...
	 * returned instance has same field to the original one, and test for invalid
	 * edge
	 */
	@Test
	...
	public void testWrap() {
		NetworkConnection edge = NetworkConnection.wrap("12", Arrays.asList(v1, v2), 0.5);
		assertTrue(null != edge);
		assertTrue(edge.getWeight() == 0.5);
		assertTrue(edge.targetVertices().contains(v2));
		assertTrue(edge.sourceVertices().contains(v1));

		assertTrue(null == NetworkConnection.wrap("11", Arrays.asList(v1, v1), 0.5));
		assertTrue(null == NetworkConnection.wrap("12", Arrays.asList(v1, v2), -0.5));
		
		assertTrue(null == NetworkConnection.wrap("15", Arrays.asList(v1, v5), 4));
		assertTrue(null == NetworkConnection.wrap("26", Arrays.asList(v2, v6), 4));
		assertTrue(null != NetworkConnection.wrap("17", Arrays.asList(v1, v7), 32));
	}
