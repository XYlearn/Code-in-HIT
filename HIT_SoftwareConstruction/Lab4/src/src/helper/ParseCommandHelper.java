...
public class ParseCommandHelper {
	...
	protected static final Logger logger = LogUtil.getLogger(ParseCommandHelper.class);
	 ...
	 */
	protected Class<?> parseGraphType(String commandLine) throws CommandException {
		...
		try {
			...
			String logText = "Graph Type \"" + matcher.group("type") + "\" Not Found";
	 ...
	 */
	protected List<Class<?>> parseEdgeType(String commandLine) throws CommandException {
		...
		for (int i = 0; i < types.length; i++) {
			...
			try {
				...
				if (GraphWhiteListHelper.isEdgeAdoptable(cls, graphType))
					...
					String logText = "Edge Class \"" + classname + "\" Is Not Adoptable For GraphType \""
							+ graphType.getName() + "\"";
					CommandException exception = new CommandException(logText);
	...
	protected Edge parseEdgeBuild(String commandLine) throws CommandException {
		...
		if (matcher.find()) {
			...
			if (null == startVertex) {
				String logText = "Vertex with Label \"" + startVertexLabel + "\" Not Declared";
				CommandException exception = new CommandException(logText);
			...
			if (null == endVertex) {
				String logText = "Vertex with Label \"" + endVertexLabel + "\" Not Declared";
				CommandException exception = new CommandException(logText);
			...
			try {
				...
				if(index != 0) {
					logger.warning("Change Edge Label \"" + originLabel + "\" To \"" + label + "\"");
				}
				...
				if (null == edge) {
					String logText = "Invalid Aruguments for Edge Class \"" + className + "\"; Edge \"" + label
							+ "\" Will Not Be Added";
					CommandException exception = new CommandException(logText);
				...
				try {
					graph = (Graph<Vertex, Edge>)graphType.getDeclaredConstructor(String.class).newInstance("test");
				} catch (InstantiationException e) {
					assert false;
				}
				
				if (!graph.allowDuplicateEdge() && this.isDuplicateEdge(edge)) {
					String logText = "Duplicated Edge \"" + edge.getLabel() + "\" Would Not Be Added";
					CommandException exception = new CommandException(logText);
					exception.setIgnorable(true);
					logger.info(exception.getLogText());
					throw exception;
				...
				if(!graph.allowLoop() && edge.isLoop()) {
					String logText = "Loop Edge \"" + edge.getLabel() + "\" Would Not Be Added";
					CommandException exception = new CommandException(logText);
					exception.setIgnorable(true);
					logger.info(exception.getLogText());
					throw exception;
				...
				if (edge.isDirected() ^ directedText.equals("Yes")) {
					String logText = "Edge \"" + edge.getLabel() + "\" Has Been Changed To ";
					if(edge.isDirected())
						logText += "Directed";
					else
						logText += "Indirected";
	...
	protected Edge parseHyperEdgeBuild(String content) throws CommandException {
		...
		if (matcher.find()) {
			...
			try {
				tempGraph = (Graph<Vertex, Edge>) graphType.getDeclaredConstructor(String.class).newInstance("temp");
			...
			if(!tempGraph.allowHyperEdge()) {
				String logText = "HyperEdge Not Allowed In Graph Type \"" + graphType.getName() + "\"";
				CommandException exception = new CommandException(logText);
				exception.setIgnorable(true);
				logger.info(exception.getLogText());
				throw exception;
			...
			try {
				...
				String logText = "HyperEdge Class \"" + className + "\" Not Found";
				CommandException exception = new CommandException(logText);
				exception.setStartPos(matcher.start("type"));
				logger.info(exception.getLogText());
				throw exception;
			...
			for (String vertLabel : vertexLabels) {
				...
				if (null == vertex) {
					String logText = "Vertex with Label \"" + vertLabel + "\" Not Declared";
					CommandException exception = new CommandException(logText);
					int exceptionPos = matcher.group("vertices").indexOf(vertLabel) + matcher.start("vertices");
			...
			try {
				edge = (Edge) type.getDeclaredMethod("wrap", String.class, List.class, double.class).invoke(null, label,
						verts, -1);
				if (null == edge) {
					String logText = "Invalid Aruguments for HyperEdge Class \"" + className + "\"; HyperEdge \""
							+ label + "\" Will Not be added";
					CommandException exception = new CommandException(logText);
					exception.setIgnorable(true);
					exception.setStartPos(0);
					logger.info(exception.getLogText());
	 ...
	 *                throw when parse exception occurs
	 */
	protected List<Class<?>> parseVertexType(String content) throws CommandException {
		List<Class<?>> res = new ArrayList<>();
		...
		for (String typeName : typeNames) {
			...
			try {
				...
				CommandException exception = new CommandException(logText);
				int startPos = matcher.start("types") + matcher.group("types").indexOf(typeName);
		...
		if (matcher.find()) {
			...
			try {
				...
				if (!vertexTypes.contains(type)) {
					String logText = "Vertex Type \"" + className + "\" Not Declared";
					CommandException exception = new CommandException(logText);
					exception.setStartPos(matcher.start("type"));
					logger.info(exception.getLogText());
					throw exception;
			...
			try {
				Vertex vertex = (Vertex) type.getDeclaredMethod("wrap", String.class, String[].class).invoke(null,
						label, args);
				if (null == vertex) {
					String logText = "Invalid Aruguments for Vertex Class \"" + className + "\"; Vertex \"" + label
							+ "\" Build Failed";
					CommandException exception = new CommandException(logText);
					exception.setStartPos(0);
					logger.info(exception.getLogText());
				...
				if (!GraphWhiteListHelper.isVertexAdoptable(vertex.getClass(), graphType)) {
					String logText = "Invalid Vertex Class \"" + type.getName() + "\" for GraphType \""
							+ graphType.getName() + "\"";
					CommandException exception = new CommandException(logText);
					exception.setStartPos(matcher.start("type"));
					logger.info(exception.getLogText());
	 ...
	 */
	protected static boolean executeVertexAdd(Graph<Vertex, Edge> graph, String command) throws CommandException {
		...
		try {
			...
			throw exception;
				...
				| SecurityException e) {
			// reflect call failed
			String logText = "Vertex Type \"" + classname + "\" has no accessible static wrap(String, String[]) method";
			CommandException exception = new CommandException(logText);
			exception.setStartPos(matcher.start("type"));
			logger.info(exception.getLogText());
			throw exception;
	 ...
	 *                raise when command parse or execute failed
	 */
	protected static boolean executeVertexDelete(Graph<Vertex, Edge> graph, String command) throws CommandException {
			...
			try {
				if (Pattern.matches(regex, vertex.getLabel()))
					if (!graph.removeVertex(vertex)) {
						String logText = "Fail to Remove Vertex \"" + vertex.getLabel() + "\".";
						CommandException exception = new CommandException(logText);
	 ...
	 *                raise when command parse or execute failed
	 */
	protected static boolean executeEdgeAdd(Graph<Vertex, Edge> graph, String command) throws CommandException {
		...
		try {
			...
			if (!graph.containVertex(Vertex.common(vertexLabel1))) {
				String logText = "Vertex \"" + vertexLabel1 + "\" Not in Graph " + graph.getName();
				CommandException exception = new CommandException(logText);
			...
			if (!graph.containVertex(Vertex.common(vertexLabel2))) {
				String logText = "Vertex \"" + vertexLabel2 + "\" Not in Graph " + graph.getName();
				CommandException exception = new CommandException(logText);
			...
			if (edge == null || !graph.addEdge(edge)) {
				String logText = "Can't add Edge \"" + label + "\" to Graph \"" + graph.getClass().getName() + "\"";
				CommandException exception = new CommandException(logText);
				exception.setStartPos(0);
				logger.info(exception.getLogText());
				throw exception;
	 ...
	 *                raise when command parse or execute failed
	 */
	protected static boolean executeEdgeDelete(Graph<Vertex, Edge> graph, String command) throws CommandException {
			...
			try {
				if (Pattern.matches(regex, edge.getLabel()))
					...
					if (!graph.removeEdge(edge)) {
						String logText = "Fail to Remove Edge \"" + edge.getLabel() + "\".";
						CommandException exception = new CommandException(logText);
						exception.setStartPos(matcher.start("regex"));
						throw exception;
					}
	 ...
	 *                raise when command parse or execute failed
	 */
	protected static boolean executeHyperEdgeAdd(Graph<Vertex, Edge> graph, String command) throws CommandException {
		...
		try {
			...
			for (String vertexLabel : vertexLabels)
				if (!graph.containVertex(Vertex.common(vertexLabel))) {
					String logText = "Vertex \"" + vertexLabel + "\" Not in Graph \"" + graph.getName() + "\"";
					CommandException exception = new CommandException(logText);
					int startPos = matcher.start("vertices") + matcher.group("vertices").indexOf(vertexLabel);
					exception.setStartPos(startPos);
					logger.info(exception.getLogText());
					throw exception;
				}
			...
			Edge edge = (Edge) clazz.getDeclaredMethod("wrap", String.class, List.class, double.class).invoke(null,
					label, vertices, -1);
			if (edge == null || !graph.addEdge(edge)) {
				String logText = "Can't Add Edge \"" + edge.getLabel() + "\" of Type \"" + classname + "\" to Graph \""
						+ graph.getName() + "\"";
				CommandException exception = new CommandException(logText);
				exception.setStartPos(0);
				logger.info(exception.getLogText());
	 ...
	 */
	public static ConcreteGraph buildGraph(String content) throws CommandException {
	 ...
	 *                raise when parse or build failed
	 */
	public static ConcreteGraph buildGraphFromFile(String pathname) throws IOException, CommandException {
	 ...
	 */
	public static void parseAndExecuteCommand(Graph<Vertex, Edge> graph, String command) throws CommandException {
		...
		case "vertex":
			if (args[1].equals("--add")) {
				...
				String logText = "Unknown argument " + args[1] + " for vertex";
				CommandException exception = new CommandException(logText);
				exception.setLineNo(0);
				exception.setStartPos(0);
				exception.setCommand(command);
	...
	/**
	 * check if an edge is duplicated in the graph. duplicate means it has the same
	 * target and source as other edge. invalid to hyperedge
	 * 
	 * @param edge
	 *            edge to check
	...
	private boolean isDuplicateEdge(Edge edge) {
		...
		for (Edge edge2 : this.edges) {
			if (!edge2.isDirected() && vertices.containsAll(edge2.vertices()))
				return true;
			else if (edge2.isDirected() && edge.targetVertices().containsAll(edge2.targetVertices())
					&& edge.sourceVertices().containsAll(edge2.sourceVertices()))
				return true;
			else
				return false;
		}
