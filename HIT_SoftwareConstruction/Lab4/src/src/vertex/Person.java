...
public class Person extends Vertex {
	...
	/**
	 * AF: label:String represent Person's name; sex:String represent Person's sex;
	 * age:int represent Person's age.
	 * 
	 * RI: age >= 0; sex == "M" or sex == "F"
	 * 
	 * safety from rep exposure: label is unmodifiable, the sex can't be set again
	 * after wrap
	 */
	 ...
	 * @param age
	 *            Person's age
	 * @return return true if age set successfully. if age is invalid return false
	 */
	...
	/**
	 * Fill detail information of person. Information include the sex and age of the
	 * Person. If the sex has been set, it will not be modified again and be
	 * ignored.
	 * 
	 * @param args
	 *            args[0] represents sex, either 'M'(for male) or 'F'(for female).
	 *            args[1] represents age, should be string of a non-negative number
	...
	protected static Person wrap(Class<?> type, String label, String[] args) {
		Person person;
		try {
			person = (Person) type.getDeclaredConstructor(String.class).newInstance(label);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	 ...
	 * @return return generated Person Instance if age >= 0. else return null.
	 */
	protected static Person wrap(Class<?> type, String name, boolean isMale, int age) {
