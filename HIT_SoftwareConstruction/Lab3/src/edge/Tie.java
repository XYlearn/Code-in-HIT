...
public class Tie extends Edge {
	...
	/**
	 * AF : see Parent Class. vertices.get(0) is source vertex, vertices.get(1) is
	 * target vertex
	 * 
	 * RI : vertices is and ArrayList, vertices.size() == 2 and vertices.get(0) not
	 * equal to vertices.get(1)
	 * 
	...
	/**
	 * Add vertices to Tie, the first element of vertices is source vertex of edge,
	 * and the second element of vertices is target vertex of edge. if the source
	 * and target has been set or the vertices doesn't correspond to
	...
	public boolean valid() {
		return vertices.size() == 2 && new HashSet<>(vertices).size() == 2 && getWeight() > 0 && getWeight() <= 1;
	}
	...
	public Edge clone() {
		return wrap(this.getClass(), getLabel(),
				Arrays.asList(sourceVertices().iterator().next(), targetVertices().iterator().next()),
				this.getWeight());
	}
	 ...
	 *         match {@link Tie} description or the type is not Found, function will
	 *         fail and return null.
	 */
	protected static Tie wrap(Class<? extends Tie> type, String label, List<Vertex> vertices, double weight) {
		if (vertices.size() != 2 && new HashSet<>(vertices).size() != 2)
			return null;
		try {
			// construct instance of type
			Tie tie = type.getDeclaredConstructor(String.class, double.class).newInstance(label, weight);
