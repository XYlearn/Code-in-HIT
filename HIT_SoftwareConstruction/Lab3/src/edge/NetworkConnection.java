...
public class NetworkConnection extends Edge {
	...
	/**
	 * AF : represents a general edge. the edge has a label information, weight and
	 * vertices it connect.label:String; ip:String; weight:double; vertices:HashSet
	 * 
	 * RI : Is directed and weight is non-negative and the two points of edge can't
	 * be the same or are both Instance of Server or are both Instance of Computer.
	 * 
	 * safety from rep exposure: defensive copy and can't add additional vertices
	 * after wrap
	 */
	 ...
	 * @return true if vertices added successfully, return false if the edge is not
	 *         valid after add operation
	 */
	@Override
	@Deprecated
	 ...
	 *            edge's label
	 * @param vertices
	 *            vertices of the edge, size must be 2 and fit the representation in
	 *            class document
	 * @param weight
	 *            weight of the edge, must be positive
	 * @return return new Instance of edge if the arguments are valid. else return
	 *         null
	 */
	public static NetworkConnection wrap(String label, List<Vertex> vertices, double weight) {
		if (vertices.size() != 2)
			return null;
		NetworkConnection edge = new NetworkConnection(label, weight);
		if (edge.addVertices(Arrays.asList(vertices.get(0).clone(), vertices.get(1).clone()))) {
			if (edge.valid())
				return edge;
			else
				return null;
	...
	/**
	 * check if two vertices are connectable. Two vertices are connectable only if
	 * they are instance of {@link Computer}, {@link Router} or {@link Server} and
	 * can't be equal to each other .In addition, {@link Computer} can't connect to
	 * {@link Computer} directly, same to {@link Server}.
	 * 
	 * @param source
	 *            source Vertex
	 * @param target
	...
	protected static boolean connectable(Vertex source, Vertex target) {
		return !(source.equals(target) || (source instanceof Computer && target instanceof Computer)
