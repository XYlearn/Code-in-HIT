...
public class IpVertex extends Vertex {
	...
	/**
	 * Fill in IP information to Vertex.If the IP has already existed, it will be
	 * updated
	 * 
	 * @param args:
	...
	public String toString() {
		return ParseUtil.getClassName(this.getClass()) + "(" + label + " , " + ip + ")";
	}
	 ...
	 */
	protected static IpVertex wrap(Class<? extends IpVertex> type, String label, String[] args) {
		if (args.length < 1)
			return null;
		IpVertex vertex;
		try {
			vertex = type.getDeclaredConstructor(String.class).newInstance(label);
		} catch (Exception e1) {
			e1.printStackTrace();
			return null;
		}
	 ...
	 */
	protected static IpVertex wrap(Class<? extends IpVertex> type, String name, String ip) {
