...
template<typename T, typename W>
MTGraph<T> &AdjGraph<T, W>::convert2MT()
{
	...
	for (size_t i = 0; i != vertexes.size(); ++i)
	{
		if (_is_empty_index(i))
			continue;
		for (auto p = vertexes[i].first; p != nullptr; p = p->next)
		{
			graph->addEdge(i, p->adj);
		}
	}
	return *graph;
}
...
template<typename T, typename W>
OLGraph<T, W> &AdjGraph<T, W>::convert2OL()
{
	OLGraph<T, W> *graph = new OLGraph<T,W>();
	for (size_t i = 0; i != vertexes.size(); ++i)
	{
		if (_is_empty_index(i))
		{
